...
void ricerca()
{
    ...
    {
        rubrica temp;
        long codic,c=0;     //!codice-->ID da ricercare      c-->contatore
        ...
        if(codic<=c)
        {
            ...
            file.clear();       //!cancello
            file.seekg(codic*sizeof(temp),ios::beg);    //!ios::beg-->lo fa partire dalla prima posizione sizeof(temp)-->lunghezza Rubrica codic-->codice da ricercare
            file.read((char*)&temp,sizeof(temp));       //!attraverso il metodo read carico il file sulla struct
            cout<<temp.codiceContatto<<": "<<temp.n_telefono<<"\t"<<temp.cognome<<" "<<temp.nome<<endl; //!vado a leggere il file
        }
        else
